...
describe("extrairRegua — os pares saem do globals.css, nunca de lista à mão", () => {
  it("acha os dois temas, a rampa do produto e os neutros", () => {
    ...
    expect(REGUA.escuro.neutros[9]).toBe("#10100e");
  ...
  it("reproduz as razões medidas à mão no design system", () => {
    ...
    expect(razao("--color-accent", "--color-bg")).toBeCloseTo(5.80, 2);
    expect(razao(":focus-visible/outline", "--color-bg")).toBeCloseTo(4.00, 2);
    expect(razao(":focus-visible/outline", "--color-surface-elevated")).toBeCloseTo(3.66, 2);
  });
