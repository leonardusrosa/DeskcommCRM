 ...
 *
 * Fail-closed non-clinical boundary security guard.
 * Prevents any accidental or malicious ingestion of protected health information (PHI),
 * dental records, odontograms, diagnostics, radiology, or financial invoices.
 */
...
export class ClinicalBoundaryViolationError extends Error {
  ...
  constructor(field: string, context?: string) {
    super(
      `[PMS Clinical Boundary Violation] Prohibited clinical or billing field "${field}" encountered in ${
        context ?? "payload"
...
export function assertAdministrativePayloadSafe(payload: unknown, context?: string): void {
  ...
  if (Array.isArray(payload)) {
    for (const item of payload) {
      assertAdministrativePayloadSafe(item, context);
    }
    return;
  }
  ...
  for (const key of Object.keys(record)) {
    ...
    if (val && typeof val === "object") {
      assertAdministrativePayloadSafe(val, `${context ? context + "." : ""}${key}`);
    }
  }
}

export function sanitizeAdministrativeContact(raw: Record<string, unknown>): AdministrativeContact {
  assertAdministrativePayloadSafe(raw, "contact");
...
export function sanitizeAdministrativeAppointment(
  ...
  assertAdministrativePayloadSafe(raw, "appointment");
