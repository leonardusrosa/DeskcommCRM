...
import { OUTRO_VIEWPORT_GEOMETRY } from '../geometry';
...
export const SceneOutro: React.FC<SceneOutroProps> = ({ content }) => {
  ...
  const colombiaCard = OUTRO_VIEWPORT_GEOMETRY.colombiaCard;
  const cardCenterX = Math.round(colombiaCard.left + colombiaCard.width / 2);
  const cardCenterY = Math.round(colombiaCard.top + colombiaCard.height / 2);
  ...
  // Cursor Phase 1 (frames 0-90): points directly to Colombia card center
  // Cursor Phase 2 (frames 95-200): moves directly to the CTA button center (810, 500) and clicks
  const cursorX = interpolate(
    frame,
    [0, 30, 65, 95, 135, 160],
    [650, 650, cardCenterX, cardCenterX, 810, 810],
    { extrapolateRight: 'clamp' }
  );
  const cursorY = interpolate(
    frame,
    [0, 30, 65, 95, 135, 160],
    [480, 480, cardCenterY, cardCenterY, 500, 500],
    { extrapolateRight: 'clamp' }
  );
        ...
        <BrowserFrame url="demo.deskcomm.autocora.com.br/demo">
          <div className="relative h-[720px] w-full overflow-hidden">
            ...
            <div
              ...
              style={{
                left: colombiaCard.left,
                top: colombiaCard.top,
                width: colombiaCard.width,
                height: colombiaCard.height,
